	...
	var spfwc_payment = {
		...
		init: function() {
			...
			$( document.body ).on( 'click', '#place_order', function() {
				if ( spfwc_payment.isSonyPaymentChosen() ) {
		...
		isSonyPaymentChosen: function() {
			return $( '#payment_method_sonypayment-light' ).prop( 'checked' );
		},

		isSonyPaymentCvsChosen: function() {
			return $( '#payment_method_sonypayment-light_cvs' ).prop( 'checked' );
		},

		getCardMemberOption: function() {
			return $( 'input[name="sonypayment-light_card_member_option"]:checked' ).val();
		},
		...
		getPayType: function() {
			var paytype = '01';
			if ( ! $( '#sonypayment-light-card-paytype-default' ).prop( 'disabled' ) ) {
				paytype = $( '#sonypayment-light-card-paytype-default option:selected' ).val();
			} else if ( ! $( '#sonypayment-light-card-paytype-4535' ).prop( 'disabled' ) ) {
				paytype = $( '#sonypayment-light-card-paytype-4535 option:selected' ).val();
			} else if ( ! $( '#sonypayment-light-card-paytype-37' ).prop( 'disabled' ) ) {
				paytype = $( '#sonypayment-light-card-paytype-37 option:selected' ).val();
			} else if ( ! $( '#sonypayment-light-card-paytype-36' ).prop( 'disabled' ) ) {
				paytype = $( '#sonypayment-light-card-paytype-36 option:selected' ).val();
			}
			return paytype;
		},
		...
		getToken: function() {
			...
			var check = true;
			if ( ! isNumberString( $( '#sonypayment-light-card-number' ).val() ) ) {
				check = false;
			}
			if ( ! isNumberString( $( '#sonypayment-light-card-expmm' ).val() ) ) {
				check = false;
			}
			if ( ! isNumberString( $( '#sonypayment-light-card-expyy' ).val() ) ) {
				check = false;
			}
			if ( $( '#sonypayment-light-card-seccd' ).val() != undefined ) {
				if ( ! isNumberString( $( '#sonypayment-light-card-seccd' ).val() ) ) {
					check = false;
				}
			}
			if ( ! isValidInput( $( '#sonypayment-light-card-name' ).val() ) ) {
				check = false;
			}
			if ( ! check ) {
			...
			var cardno = $( '#sonypayment-light-card-number' ).val();
			var expmm = $( '#sonypayment-light-card-expmm' ).val();
			var expyy = $( '#sonypayment-light-card-expyy' ).val();
			var seccd = ( $( '#sonypayment-light-card-seccd' ).val() != undefined ) ? $( '#sonypayment-light-card-seccd' ).val() : '';
...
let spfwc_light_payment_block_page = false;

function setToken( token, card ) {
	document.getElementById( "sonypayment-token-code" ).value = token;
	document.getElementById( "sonypayment-billing-name" ).value = document.getElementById( "sonypayment-light-card-name" ).value;
	if( spfwc_light_payment_block_page ) {
		return;
	}
