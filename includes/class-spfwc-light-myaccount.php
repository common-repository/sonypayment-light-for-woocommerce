...
class SPFWC_MyAccount {
	...
	public function account_menu_items( $items ) {
		...
		if ( isset( $settings['cardmember'] ) && 'yes' === $settings['cardmember'] ) {
			...
			if ( $member->is_card_member() ) {
				$items['edit-cardmember'] = __( 'Update credit card', 'sonypayment-light-for-woocommerce' );
			} else {
				$items['edit-cardmember'] = __( 'Save credit card', 'sonypayment-light-for-woocommerce' );
			}
		}
		return $items;
	}
	...
	public function account_edit_endpoint() {
		...
		include SPFWC_LIGHT_PLUGIN_DIR . '/includes/spfwc-light-form-cardmember.php';
	}
	...
	public function get_endpoint_title( $title ) {
		...
		$member      = new SPFWC_Card_Member( $customer_id );
		if ( $member->is_card_member() ) {
			$title = __( 'Update credit card', 'sonypayment-light-for-woocommerce' );
		} else {
			$title = __( 'Save credit card', 'sonypayment-light-for-woocommerce' );
		}
		return $title;
	}
		...
		if ( is_user_logged_in() && 'yes' === $settings['cardmember'] ) {
			...
			if ( isset( $_POST['done3d_myaccount'] ) ) {
				...
				if ( empty( $_POST['sonypayment_card_number'] ) ) {
					wc_add_notice( sprintf( __( '%s is a required field.', 'sonypayment-light-for-woocommerce' ), __( 'Card number', 'sonypayment-light-for-woocommerce' ) ), 'error' );
				}

				if ( empty( $_POST['sonypayment_card_expmm'] ) || empty( $_POST['sonypayment_card_expyy'] ) ) {
					wc_add_notice( sprintf( __( '%s is a required field.', 'sonypayment-light-for-woocommerce' ), __( 'Expiry (MM/YY)', 'sonypayment-light-for-woocommerce' ) ), 'error' );
				}

				if ( isset( $settings['seccd'] ) && 'yes' === $settings['seccd'] ) {
					if ( empty( $_POST['sonypayment_card_seccd'] ) ) {
						wc_add_notice( sprintf( __( '%s is a required field.', 'sonypayment-light-for-woocommerce' ), __( 'Card code', 'sonypayment-light-for-woocommerce' ) ), 'error' );
					}
				}

				if ( empty( $_POST['sonypayment_card_name'] ) ) {
					wc_add_notice( sprintf( __( '%s is a required field.', 'sonypayment-light-for-woocommerce' ), __( 'Card name', 'sonypayment-light-for-woocommerce' ) ), 'error' );
				}
				...
				if ( 0 === wc_notice_count( 'error' ) ) {
					...
					if ( isset( $_POST['transaction_code'] ) ) {
						$param_list['MerchantFree1'] = filter_input( INPUT_POST, 'transaction_code' );
					}
					try {
						...
						if ( ! empty( $token ) ) {
							...
							if ( 'OK' !== $response_token['ResponseCd'] || 'OK' !== $response_token['TokenResponseCd'] ) {
								...
								$localized_message = __( 'Update processing failed. Please retry.', 'sonypayment-light-for-woocommerce' );
				...
				if ( is_user_logged_in() && 'yes' === $settings['cardmember'] ) {
					...
					if ( $member->is_card_member() ) {
						...
						if ( 'OK' === $response_member['ResponseCd'] ) {
							...
							if ( 'OK' === $response_member['ResponseCd'] ) {
								wc_add_notice( __( 'Card member updated successfully.', 'sonypayment-light-for-woocommerce' ) );
							} else {
						...
						if ( 'OK' === $response_member['ResponseCd'] ) {
							wc_add_notice( __( 'Card member saved successfully.', 'sonypayment-light-for-woocommerce' ) );
						} else {
							$responsecd = explode( '|', $response_member['ResponseCd'] );
							foreach ( (array) $responsecd as $cd ) {
								$response_member[ $cd ] = SPFWC_Payment_Message::response_message( $cd );
	...
	public function delete_cardmember() {
		...
		try {
				...
				if ( $member->is_card_member() ) {
					...
					if ( 'OK' === $response_member['ResponseCd'] ) {
						...
						if ( 'OK' !== $response_member['ResponseCd'] ) {
							...
							$localized_message = __( 'Failed deleting card member.', 'sonypayment-light-for-woocommerce' );
							throw new SPFWC_Exception( print_r( $response_member, true ), $localized_message );
						}
					} else {
						$responsecd = explode( '|', $response_member['ResponseCd'] );
			...
			wc_add_notice( __( 'Card member deleted successfully.', 'sonypayment-light-for-woocommerce' ) );
	...
	public function account_scripts() {
		...
		wp_register_script( 'sonypayment_script', plugins_url( 'assets/js/spfwc-light-myaccount.js', SPFWC_LIGHT_PLUGIN_FILE ), array(), SPFWC_LIGHT_VERSION, true );
		$customer_id                             = get_current_user_id();
		$member                                  = new SPFWC_Card_Member( $customer_id );
		...
		$sonypayment_params['message']           = array(
			...
			'confirm_delete'    => __( 'Are you sure you want to delete card member?', 'sonypayment-light-for-woocommerce' ),
