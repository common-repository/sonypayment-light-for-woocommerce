...
<form name="sonypayment_cardmember" class="spfwc-edit-cardmember-form" action="" method="post">
	...
	<fieldset id="sonypayment-card-form" class="wc-payment-form" style="background:transparent;">
		...
		<p class="form-row form-row-wide">
			<label for="sonypayment-card-member-cardlast4" style="display:inline;"><?php esc_html_e( 'Last 4 digits of the saved card number: ', 'sonypayment-light-for-woocommerce' ); ?></label>
			<span id="sonypayment-card-member-cardlast4"><?php echo esc_html( $cardlast4 ); ?></span>
		</p>
		<?php endif; ?>
		<p class="form-row form-row-wide">
			<label for="sonypayment-card-number"><?php esc_html_e( 'Card number', 'sonypayment-light-for-woocommerce' ); ?> <span class="required">*</span></label>
		...
		<p class="form-row form-row-wide">
			<label><?php esc_html_e( 'Expiry (MM/YY)', 'sonypayment-light-for-woocommerce' ); ?> <span class="required">*</span></label>
			<input id="sonypayment-card-expmm" name="sonypayment_card_expmm" class="input-text" inputmode="numeric" autocomplete="off" autocorrect="no" autocapitalize="no" spellcheck="no" type="tel" maxlength="2" style="width:60px" placeholder="<?php esc_attr_e( 'MM', 'sonypayment-light-for-woocommerce' ); ?>" />&nbsp;/&nbsp;
			<input id="sonypayment-card-expyy" name="sonypayment_card_expyy" class="input-text" inputmode="numeric" autocomplete="off" autocorrect="no" autocapitalize="no" spellcheck="no" type="tel" maxlength="2" style="width:60px" placeholder="<?php esc_attr_e( 'YY', 'sonypayment-light-for-woocommerce' ); ?>" />
		</p>
		<?php if ( ! empty( $settings['seccd'] ) && 'yes' === $settings['seccd'] ) : ?>
		<p class="form-row form-row-wide">
			<label for="sonypayment-card-seccd"><?php esc_html_e( 'Card code', 'sonypayment-light-for-woocommerce' ); ?> <span class="required">*</span></label>
		...
		<?php endif; ?>
		<p class="form-row form-row-wide">
			<label for="sonypayment-card-name"><?php esc_html_e( 'Card name', 'sonypayment-light-for-woocommerce' ); ?> <span class="required">*</span></label>
	...
	<fieldset id="sonypayment-consent-message">
		...
		<p class="sonypayment-consent-area">
			<input type="checkbox" id="sonypayment_agree" value="agree" /><label for="sonypayment_agree"><span><?php esc_html_e( 'I agree to the handling of personal information', 'sonypayment-light-for-woocommerce' ); ?></span></label>
		</p>
	...
	<p>
		...
		<button type="submit" class="woocommerce-Button button" id="save-cardmember" value="save"><?php esc_html_e( 'Save changes', 'sonypayment-light-for-woocommerce' ); ?></button>
		<?php if ( ! empty( $cardlast4 ) && $deletable ) : ?>
		<button type="submit" class="woocommerce-Button button" id="delete-cardmember" value="delete"><?php esc_html_e( 'Delete the saved card member', 'sonypayment-light-for-woocommerce' ); ?></button>
		<?php endif; ?>
