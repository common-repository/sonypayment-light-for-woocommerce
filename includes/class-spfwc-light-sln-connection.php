...
class SPFWC_SLN_Connection {
	...
	public function send_request( &$param_list = array() ) {
		...
		if ( empty( $param_list ) === false ) {
			...
			if ( false === $fp ) {
				SPFWC_Logger::add_log( 'SonyPayment SLN Connection error: ' . __( 'TLS 1.2 connection failed.', 'sonypayment-light-for-woocommerce' ) );
				return $return_value;
			}
				...
				if ( true === $result ) {
					...
					if ( true === $result ) {
						if ( true !== $hm['timed_out'] ) {
							...
							SPFWC_Logger::add_log( 'SonyPayment SLN Connection error: ' . __( 'Timeout occurred during communication.', 'sonypayment-light-for-woocommerce' ) );
						}
					} else {
						SPFWC_Logger::add_log( 'SonyPayment SLN Connection error: ' . __( 'Failed to disconnect from SLN.', 'sonypayment-light-for-woocommerce' ) );
					}
				} else {
					SPFWC_Logger::add_log( 'SonyPayment SLN Connection error: ' . __( 'Timeout setting failed.', 'sonypayment-light-for-woocommerce' ) );
				}
			}
		} else {
			SPFWC_Logger::add_log( 'SonyPayment SLN Connection error: ' . __( 'Invalid request parameter specification.', 'sonypayment-light-for-woocommerce' ) );
		}
