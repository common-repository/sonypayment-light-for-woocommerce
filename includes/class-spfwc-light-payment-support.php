...
class WC_Payment_Support {
	...
	public function add_card_member_fields( $user ) {
		...
		if ( $member->is_card_member() ) {
			...
			?>
		<h2><?php esc_html_e( 'SonyPayment Card Member', 'sonypayment-light-for-woocommerce' ); ?></h2>
		<table class="form-table" id="fieldset-sonypayment-card-member'">
			<?php if ( ! empty( $cardlast4 ) ) : ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Last 4 digits of the saved card number', 'sonypayment-light-for-woocommerce' ); ?></th>
				<td><span id="sonypayment-card-member-cardlast4"><?php echo esc_html( $cardlast4 ); ?></span></td>
			</tr>
		<?php endif; ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Delete the saved card member', 'sonypayment-light-for-woocommerce' ); ?></th>
				<td>
					<label for="sonypayment-card-member-delete"><input name="sonypayment_card_member_delete" type="checkbox" id="sonypayment-card-member-delete" value="false"><?php esc_html_e( 'Delete a card member information registered in SonyPayment.', 'sonypayment-light-for-woocommerce' ); ?></label>
					<br/>
					<span class="description"><?php esc_html_e( 'Don\'t delete in case of the sales isn\'t recorded or the subscription products is ongoing.', 'sonypayment-light-for-woocommerce' ); ?></span>
				</td>
			</tr>
		</table>
			<?php
		...
		foreach ( $fields as $key => $field ) {
			if ( strpos( $key, 'company' ) !== false ) {
				$address_fields['last_name_kana']  = array(
					'label'    => __( 'Last name (kana)', 'sonypayment-light-for-woocommerce' ),
					'required' => false,
					'class'    => array( 'form-row-first' ),
				);
				$address_fields['first_name_kana'] = array(
					'label'    => __( 'First name (kana)', 'sonypayment-light-for-woocommerce' ),
					'required' => false,
