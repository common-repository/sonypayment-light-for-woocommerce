...
final class SPFWC_Blocks_Support extends AbstractPaymentMethodType {
	 ...
	 * @var string
	 */
	protected $name = 'sonypayment-light';

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$this->settings = get_option( 'woocommerce_sonypayment-light_settings', [] );
	...
	public function get_payment_method_script_handles() {
		...
		$script_asset_path = SPFWC_LIGHT_PLUGIN_DIR . '/build/index.asset.php';
		...
		$script_url        = SPFWC_LIGHT_PLUGIN_URL . $script_path;

		wp_register_script(
			'wc-sonypayment-light-payments-blocks',
			$script_url,
...
//			wp_set_script_translations( 'wc-sonypayment-light-payments-blocks', 'sonypayment-light-for-woocommerce', SPFWC_LIGHT_PLUGIN_URL . '/languages');
			wp_set_script_translations( 'wc-sonypayment-light-payments-blocks', 'sonypayment-light-for-woocommerce');
		}

		return [ 'wc-sonypayment-light-payments-blocks' ];
	}
	...
	public function get_payment_method_data() {
		...
		if ( is_page( $checkout_page_id ) ) {
			...
			if ( isset( $_GET['order_id'] ) && isset( $_GET['key'] ) ) {
				...
				$response_data_3d = $order->get_meta( 'wc_sonypayment-light_block_process_payment_retransfer_' . $key, true );
				if ( $response_data_3d ) {
					$order->delete_meta_data( 'wc_sonypayment-light_block_process_payment_retransfer_' . $key );
					$order->save();
				} else {
