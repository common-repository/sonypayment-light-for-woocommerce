<?php
/**
 * WC_Gateway_SonyPayment_light_CVS class.
 *
 ...
 * @since 1.0.0
 */
class WC_Gateway_SonyPayment_light_CVS extends WC_Payment_Gateway {

	/**
	 * Constructor for the gateway.
	 */
	public function __construct() {
		$this->id                 = 'sonypayment-light_cvs';
		$this->has_fields         = true;
		$this->method_title       = __( 'SonyPayment Online Payment Collection Agency Service', 'sonypayment-light-for-woocommerce' );
		$this->method_description = __( 'When you contract with Sony Payment Services, the payment of convenience store, bank ATM, online banking will be available. Note that sole proprietorship can not be accepted.', 'sonypayment-light-for-woocommerce' );
		$this->supports           = array(
			'products',
			'refunds',
		);
		...
		$this->order_button_text = __( 'Continue to payment', 'sonypayment-light-for-woocommerce' );
		...
		$this->form_fields = apply_filters(
			...
			array(
				'enabled'              => array(
					...
					'type'    => 'checkbox',
					'label'   => __( 'Enable SonyPayment Online Payment Collection Agency Service', 'sonypayment-light-for-woocommerce' ),
					'default' => 'no',
				),
				'title'                => array(
					...
					'default'     => __( 'Convenience store / Electronic money / Pay-easy', 'sonypayment-light-for-woocommerce' ),
					'desc_tip'    => true,
				),
				'description'          => array(
					...
					'default'     => __( 'Pay by convenience store / Electronic money / Pay-easy', 'sonypayment-light-for-woocommerce' ),
				),
				'testmode'             => array(
				...
				'paylimit'             => array(
					'title'       => __( 'Due date for payment', 'sonypayment-light-for-woocommerce' ),
					'type'        => 'text',
					'description' => __( 'Enter \'Due date for payment\'. The date on which the number of days specified from the purchase has passed is the payment deadline.', 'sonypayment-light-for-woocommerce' ),
					'default'     => '7',
					'desc_tip'    => true,
					'placeholder' => '',
				),
				'settlement_fee'       => array(
					'title'       => __( 'Settlement fee', 'sonypayment-light-for-woocommerce' ),
					'type'        => 'checkbox',
					'label'       => __( 'Set the settlement fee', 'sonypayment-light-for-woocommerce' ),
					'desc_tip'    => true,
					'description' => __( 'It will be added to the total amount as a settlement fee.', 'sonypayment-light-for-woocommerce' ),
					'default'     => 'no',
				),
				'settlement_fee_table' => array(
					'type'        => 'hidden',
					'default'     => '',
					'description' => __( 'Please set the settlement fee by the amount including tax. In case of the the online payment collection agency, the settlement fee is added to the total amount which is included shipping and consumption tax.', 'sonypayment-light-for-woocommerce' ),
				),
				'order_status'         => array(
					'title'       => __( 'Order Status', 'sonypayment-light-for-woocommerce' ),
					'type'        => 'select',
					'options'     => array(
						'processing' => __( 'Set to \'Processing\' after payment is received', 'sonypayment-light-for-woocommerce' ),
						'completed'  => __( 'Set to \'Completed\' after payment is received', 'sonypayment-light-for-woocommerce' ),
					),
					'default'     => 'processing',
					'description' => __( 'If a non-virtual item is purchased, it will be \'Processing\' even if \'Completed\' is selected.', 'sonypayment-light-for-woocommerce' ),
				),
				'logging'              => array(
	...
	public function payment_fields() {
		...
		if ( $this->settlement_fee ) {
			...
			if ( isset( $cart_totals['total'] ) ) {
				...
				if ( 0 < $fee ) {
					echo '<div class="sonypayment-cvs-settlement-fee">' . sprintf( __( '* A settlement fee of <strong>%s</strong> will be added at the time of settlement.', 'sonypayment-light-for-woocommerce' ), wc_price( $fee ) ) . '</div>';
				}
			}
		}
		echo '</div>';
	...
	public function payment_scripts() {
		...
		wp_register_script( 'sonypayment_cvs_script', plugins_url( 'assets/js/spfwc-light-payment-cvs.js', SPFWC_LIGHT_PLUGIN_FILE ), array(), SPFWC_LIGHT_VERSION, true );
		$sonypayment_cvs_params            = array();
		$sonypayment_cvs_params['message'] = array(
			'error_billing_last_name_kana'  => __( 'Please enter billing last name (kana).', 'sonypayment-light-for-woocommerce' ),
			'error_billing_first_name_kana' => __( 'Please enter billing first name (kana).', 'sonypayment-light-for-woocommerce' ),
		);
		...
		try {
			if ( $order->get_total() > 0 ) {
				...
				if ( 1 < count( $order_items ) ) {
					if ( 16 < mb_strlen( $item_name . __( ' etc.', 'sonypayment-light-for-woocommerce' ), 'UTF-8' ) ) {
						$item_name = mb_substr( $item_name, 0, 12, 'UTF-8' ) . __( ' etc.', 'sonypayment-light-for-woocommerce' );
					}
				} else {
					if ( 16 < mb_strlen( $item_name, 'UTF-8' ) ) {
						$item_name = mb_substr( $item_name, 0, 13, 'UTF-8' ) . __( '...', 'sonypayment-light-for-woocommerce' );
					}
				}
				...
				if ( $this->settlement_fee ) {
					...
					if ( $fee ) {
						...
						$item_fee->set_name( __( 'Settlement fee', 'sonypayment-light-for-woocommerce' ) );
	...
	public function process_response( $response_data, $order, $params ) {
		...
		if ( 'OK' === $response_data['ResponseCd'] && ! empty( $response_data['MerchantFree1'] ) ) {
			...
			$order->update_status( 'on-hold', __( 'Awaiting payment of online', 'sonypayment-light-for-woocommerce' ) );

			// $message = __( 'Payment procedure is completed.', 'sonypayment-light-for-woocommerce' );
	...
	public function email_payment_detail( $order, $sent_to_admin, $plain_text, $email ) {
		...
		if ( $plain_text ) {
			$payment_detail  = "\n" . esc_html__( 'Payment method', 'sonypayment-light-for-woocommerce' ) . "\n\n";
			$payment_detail .= esc_html__( 'If the payment has not been completed yet, please proceed the payment process from the following URL.', 'sonypayment-light-for-woocommerce' ) . "\n";
			$payment_detail .= esc_html__( 'Payment URL', 'sonypayment-light-for-woocommerce' ) . ':' . esc_url( $url ) . "\n";
			$payment_detail .= esc_html__( 'The due date of the payment', 'sonypayment-light-for-woocommerce' ) . ':' . esc_html( $paylimit_formatted ) . "\n";
		} else {
			...
			$payment_detail = '<table id="spfwc-email-payment-detail" cellspacing="0" cellpadding="0" style="width: 100%; vertical-align: top; margin-bottom: 40px; padding:0;" border="0">
				...
				<td style="text-align:' . $text_align . '; font-family: \'Helvetica Neue\', Helvetica, Roboto, Arial, sans-serif; border:0; padding:0;" valign="top" width="50%">
					<h2>' . esc_html__( 'Payment method', 'sonypayment-light-for-woocommerce' ) . '</h2>
					<div style="padding:12px;color:#636363;border:1px solid #e5e5e5">
						<div>
							<span>' . __( 'If the payment has not been completed yet, please proceed the payment process from the following URL.', 'sonypayment-light-for-woocommerce' ) . '</span>
						</div>
						<div>
							<strong>' . esc_html__( 'Payment URL', 'sonypayment-light-for-woocommerce' ) . ':</strong> <span>' . esc_html( $url ) . '</span>
						</div>
						<div>
							<strong>' . esc_html__( 'The due date of the payment', 'sonypayment-light-for-woocommerce' ) . ':</strong> <span>' . esc_html( $paylimit_formatted ) . '</span>
						</div>
					</div>
				</td></tr>
			</table>';
